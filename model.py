...
if __name__ == "__main__":
    ...
    probs_one = calculate_cuisine_probabilities(valid_one, prior[MONDAY])
    if probs_one:
        user_feedback_loop(probs_one, valid_one, prior[MONDAY])

    print("\nFor Fridge Two (Tuesday):")
    probs_two = calculate_cuisine_probabilities(valid_two, prior[TUESDAY])
    if probs_two:
        user_feedback_loop(probs_two, valid_two, prior[TUESDAY])

    print("\nFor Fridge Three (Wednesday):")
    probs_three = calculate_cuisine_probabilities(valid_three, prior[WEDNESDAY])
    if probs_three:
        user_feedback_loop(probs_three, valid_three, prior[WEDNESDAY])
'''
